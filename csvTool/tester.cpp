...
typedef std::vector<CsvRow> CSV;

CSV read_all(std::ifstream& inFS, char d=',', char inEOL='\n') {
  std::string field;
  CsvParser p;
  CSV csv;
  CsvRow row;
  p.delim = d;
  p.inEOL = inEOL;
  while (p.getRow(inFS, field, row))  {
    CsvRow nextRow;
    nextRow.swap(row);
    csv.push_back(std::move(nextRow));
    row.clear();
  }
  return csv;
}
